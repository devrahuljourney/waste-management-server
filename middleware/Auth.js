...
exports.isUser = async (req, res) => {
    try {
        if(req.user.role !== "User"){
            return res.status(401).json({
                success: false,
                message : "This is protected route valid only for user"
            })
        }
        next();
    } catch (error) {
...
exports.PickUpBoy = async (req, res) => {
    try {
        if(req.user.role !== "PickUpBoy"){
            return res.status(401).json({
                success: false,
                message : "This is protected route valid only for PickUp Boy"
            })
        }
        next();
    } catch (error) {
